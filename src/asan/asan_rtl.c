...
static inline void misan_set_shadow(uintptr_t addr, size_t size, uint8_t val) {
    uint8_t *shadow = (uint8_t *)((addr >> 3) + ASAN_SHADOW_BASE);
    for (size_t i = 0; i < (size >> 3); ++i) {
        shadow[i] = val;
    }
}

// ASan shadow fill stubs generated by Clang
void __asan_set_shadow_00(uintptr_t addr, size_t size) { misan_set_shadow(addr, size, 0x00); }
void __asan_set_shadow_f1(uintptr_t addr, size_t size) { misan_set_shadow(addr, size, 0xf1); }
void __asan_set_shadow_f2(uintptr_t addr, size_t size) { misan_set_shadow(addr, size, 0xf2); }
void __asan_set_shadow_f3(uintptr_t addr, size_t size) { misan_set_shadow(addr, size, 0xf3); }
void __asan_set_shadow_f5(uintptr_t addr, size_t size) { misan_set_shadow(addr, size, 0xf5); }
void __asan_set_shadow_f8(uintptr_t addr, size_t size) { misan_set_shadow(addr, size, 0xf8); }
